...
type NfeTypeWithAction = NfeType & {
  action?: string
}
...
// Column Definitions
const columnHelper = createColumnHelper<NfeTypeWithAction>()

const InvoiceListTable = ({ invoiceData }: { invoiceData: NfeType[] }) => {
  // const router = useRouter()

  // States
  // const [status, setStatus] = useState<NfeType['status']>('')
  ...
  const columns = useMemo<ColumnDef<NfeTypeWithAction, any>[]>(
    () => [
      {
        id: 'select'
          ...
          <CustomAvatar
            ...
            color={
              row.original.status === '1'
                ? invoiceStatusObj['Sent'].color
                : row.original.status === '5'
                  ? invoiceStatusObj['Paid'].color
                  : 'warning'
            }
            size={28}
          >
            <i
              className={classnames(
                'bs-4 is-4',
                row.original.status === '1'
                  ? invoiceStatusObj['Sent'].icon
                  : row.original.status === '5'
                    ? invoiceStatusObj['Paid'].icon
                    : ''
              )}
            />
          </CustomAvatar>
      ...
      columnHelper.accessor('date', {
        ...
        cell: ({ row }) => (
          <Typography>
            {row.original.date ? format(new Date(row.original.date), "dd/MM/yyyy 'às' HH:mm") : ''}
          </Typography>
        )
      }),
            ...
            <IconButton
              onClick={async () => {
                ...
                const { data } = await axios.get(
                  `${process.env.NEXT_PUBLIC_API_URL}/nfe/authorization/${row.original.id}`,
                  {
                    headers: {
                ...
                if (data) {
                  const resultNf = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/nfe`, {
                    headers: {
                      authorization: `Bearer ${login.data.token}`
                    }
                  })
                  ...
                  if (resultNf.data[0]) {
                    setData(resultNf.data)
                  }
                }
              }}
            >
              <i className='tabler-lock text-[22px] text-textSecondary' />
            </IconButton>
            <IconButton onClick={() => console.log('row', row.original.id)}>
      ...
      <CardContent className='flex justify-between flex-col items-start md:items-center md:flex-row gap-4'>
        <div className='flex items-center justify-between gap-4'>
          <div className='flex items-center gap-2'>
            <Typography className='hidden sm:block'>Exibir</Typography>
            <CustomTextField
              select
